...
class DoctorListView(APIView):
    ...
    def get(self, request, hospitalEmail):
        ...
        try:
            ...
            serializer = DoctorSerializerHospital(doctors, many=True)
            return Response(serializer.data, status=status.HTTP_200_OK)
        except Hospital.DoesNotExist:
            return Response({'error': 'Hospital not found'}, status=status.HTTP_404_NOT_FOUND)
...
class DoctorCreateView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = DoctorSerializerHospital(data=data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class HospitalDepartmentsViewHospital(APIView):
    permission_classes = [IsHospital] 
    def get(self, request, hospitalEmail):
        try:
            hospital = Hospital.objects.get(email=hospitalEmail)
...
class HospitalDepartmentsViewPatient(APIView):
    permission_classes =[IsPatient]
    def get(self, request, id):
        try:
            hospital = Hospital.objects.get(id=id)
...
def booking_notifications(request):
    ...
    try:
        # Filter bookings by patient ID and status (Cancelled or Rejected)
...
class UIDoctorListView(APIView):
    

    def get(self, request):
        doctors = Doctor.objects.all()
        serializer = DoctorSerializer(doctors, many=True, context={'request': request})  # Pass request here
        return Response({'doctors': serializer.data})
    
class UIHospitalListView(APIView):
    def get(self, request):
        hospitals = Hospital.objects.filter(is_approved=True)
        serializer = HospitalSerializers(hospitals, many=True, context={'request': request})
